...
src = 'C:\Users\USER\OneDrive\Documents\MATLAB\filtered';
dest = 'C:\Users\USER\OneDrive\Documents\MATLAB\angles';

maintemp = dir(fullfile(src,'*'));
mainfolder = setdiff({maintemp([maintemp.isdir]).name},{'.','..'});

for mainfolder_i = 1:numel(mainfolder)
    trialtemp = dir(fullfile(src,mainfolder{mainfolder_i},'*.csv'));
    trialfolder = {trialtemp(~[trialtemp.isdir]).name};
    for trialfolder_j = 1:numel(trialfolder)
        data = fullfile(src,mainfolder{mainfolder_i},trialfolder{trialfolder_j});
        outloc = fullfile(dest,strcat(mainfolder{mainfolder_i}(1:end-7),"_angles"),strcat(trialfolder{trialfolder_j}(1:end-13),"_angles.csv"));
        ...
        df2 = table(acos(res));
        df2.Properties.VariableNames = "RE_x";
        %% Flexion
        ...
        v1mag = sqrt(v1_p(:,1).^2 + v1_p(:,2).^2 + v1_p(:,3).^2);
        v1norm = v1_p./v1mag;
        res = v1norm(:,1) .* normt(:,1) + v1norm(:,2) .* normt(:,2) + v1norm(:,3) .* normt(:,3);
        ...
        v1mag = sqrt(v1_p(:,1).^2 + v1_p(:,2).^2 + v1_p(:,3).^2);
        v1norm = v1_p./v1mag;
        res = v1norm(:,1) .* flex_plane(:,1) + v1norm(:,2) .* flex_plane(:,2) + v1norm(:,3) .* flex_plane(:,3);
