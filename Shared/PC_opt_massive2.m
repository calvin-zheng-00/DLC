...
src_3d = '/fs03/sz11/calvin/categorized';
pc_src = '/fs03/sz11/calvin/PCA_coeffs';
mean_src = '/fs03/sz11/calvin/PCA_mean';
pro_src = '/fs03/sz11/calvin/Projection';
time_src = '/fs03/sz11/calvin/PCA_times';
unpro_src = '/fs03/sz11/calvin/Unprojected';
sig_src = '/fs03/sz11/calvin/Significance';
comp_src = '/fs03/sz11/calvin/Comparison';
...
allpcarr = load("/fs03/sz11/calvin/All/pconcat_coeff.mat");
allpcarr = allpcarr.coeff;
allproarr = load("/fs03/sz11/calvin/All/pconcat_project.mat");
allproarr = allproarr.score;
allmeanarr = load("/fs03/sz11/calvin/All/pconcat_mean.mat");
...
sig_all = matfile('/fs03/sz11/calvin/All/pAll_sig_80.mat');
...
for focus_cat = 1:13
    ...
    for d = 1:loops
        ...
        % elseif d == 3
        %     opt_num = 3;
        %     good_pc = [good_pc,NaN(1,loops-2)];
        %     agood_pc = [agood_pc;good_pc];
        % else
        %     opt_num = 3;
        %     temp_good_pc = agood_pc(end-2,:);
        %     temp_good_pc = temp_good_pc(~isnan(temp_good_pc));
        %     temp_good_pc = sort([temp_good_pc,good_pc]);
        %     temp_good_pc = [temp_good_pc,NaN(1,loops-d+1)];
        %     agood_pc = [agood_pc;temp_good_pc];
        %     prechose = agood_pc(end-2,:);
        %     prechose = prechose(~isnan(prechose));
        %     pcs(prechose) = [];
        end
        good_pc = [];
    ...
    outputstr = strcat('A',int2str(focus_cat),'.txt');
    writematrix(agood_pc,outputstr)
end
% figure
% b = bar(allavgerr);
