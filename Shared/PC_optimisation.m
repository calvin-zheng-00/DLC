...
while d < 4
    ...
    for r = 1:loopnum
        ...
        if d == 1
            reproarr = allproarr(:,r)*allpcarr(:,r)' + repmat(allmeanarr,height(allproarr),1);
        elseif d == 2
            curr_pc = good_pc;
            curr_pc(pcs_removed(r)) = 1;
            reproarr = allproarr(:,pcs(curr_pc))*allpcarr(:,pcs(curr_pc))' + repmat(allmeanarr,height(allproarr),1);
        elseif d == 3
            reproarr = allproarr(:,pcs(good_pc))*allpcarr(:,pcs(good_pc))' + repmat(allmeanarr,height(allproarr),1);
        end
        % reproarr = repmat(allmeanarr,height(allproarr),1);
        % reproarr = allproarr(:,[pc_order,pcs(r)])*allpcarr(:,[pc_order,pcs(r)])' + repmat(allmeanarr,height(allproarr),1);
        avgerrcomp2 = [];
    ...
    if d == 1
        %% Check if the below method works for activity set 2
        filter_pc = avgerrcomp < (prctile(avgerrcomp,75)+iqr(avgerrcomp));
        avgerrmean = avgerrcomp(filter_pc);
        good_pc = avgerrcomp < mean(avgerrmean);
        apc_order = [pcs(good_pc),0];
        pcs_removed(good_pc) = [];
        atarget = mean(avgerrmean);
        allavgerr = atarget;
        figure
        bar(avgerrcomp)
        % xticks(1:28)
        % xticklabels(string(pc_order))
    ...
    elseif d == 2
        ...
        % pc_order = [pc_order,pcs(idx)];
        if minimum < 0.9*atarget
            allavgerr = [allavgerr,minimum];
            apc_order = [apc_order,pcs_removed(idx)];
            atarget = minimum;
            good_pc(pcs_removed(idx)) = 1;
            pcs_removed(idx) = [];
        else
            d = 3;
        end
    elseif d == 3
        a = pcs(good_pc);
        fprintf(['good pcs are: ' repmat(' %d ',1,numel(a)) '\n'],a);
        d = 4;
    end
end