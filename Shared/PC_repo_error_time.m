% This file is similar to PC_repo_error, however it limits the comparison
% time window to be 2 seconds around the moment of grasp.
...
for i = 1:numel(profolder)
    ...
    %Need to make an optimisation algorithm
    ...
    for j = 1:numel(subfolder)
        ...
        fprintf('Now reading file number %d: %s\n', j, subfolder{j})
        time_window = timeTable.end_time(j)-timeTable.start_time(j);
        pre_grasp = timeTable.reach_time(j)-timeTable.start_time(j);
        all_counter = timeTable.end_time(j);
        curr_error = [];
        for t = 1:time_window
            %% Getting limb lengths
            grasp_time_3d = timeTable.grasp_time(j) - pre_grasp + t; % This is for the 3d raw data that is read file by file in each loop
            grasp_time = timeTable.reach_time(j) - pre_grasp + t; % This is for the reprojected data that is read category by category in each outer loop
            ...
            for k = 2:5
                ...
                PIP = cat(1,PIP,PIP_temp + knuckles(k,:));
                ...
                DIP = cat(1,DIP,(R*(PIP(k-1,:)-knuckles(k,:))')'.*(lengths(m+1,:)/lengths(m,:)) + PIP(k-1,:));
                R = eye(3) + sin(reproarr(grasp_time,(k-1)*3+3))*K + (1-cos(reproarr(grasp_time,(k-1)*3+3)))*K*K;
                Tip = cat(1,Tip,(R*(DIP(k-1,:)-PIP(k-1,:))')'.*(lengths(m+2,:)/lengths(m+1,:)) + DIP(k-1,:));
            end
            ...
            curr_error = cat(1,curr_error,Distance);
        end
        while height(curr_error) < height(total_error)
            curr_error = cat(1,curr_error,zeros(1,24));
        end
        total_error = cat(3,total_error,curr_error);
        flag = 1;
    end
    % figure
    % boxchart(total_error)
    % xlabel('Category')
    % ylabel('Error (mm)')
    ...
    for h = 1:24
        figure
        hold on
        title_str = char(colnames3d(h));
        for g = 1:size(total_error,3)
            plot((1:79)/40,total_error(:,h,g))
            title(title_str);
            xlabel("Time (s)");
            ylabel("Error (mm)");
        end
        saveas(gcf,strcat('Error_test\images\',mainfolder{i},'_',title_str,'_error.png'))
    end
    close all;

    % outdir = strcat("Error_time\",category);
    % if ~isfolder(outdir)
    %     mkdir("Error_time", category);
    % end
    % for h = 1:24
    %     out_error = squeeze(total_error(:,h,:));
    %     outfile = strcat(outdir,"\",char(colnames3d(h)),".mat");
    %     save(outfile,"out_error")
    % end
    flag = 1;
end

%% Plot total_error, split by joints
% for i = 1:24
%     figure
%     hold on
%     for j = 1:size(total_error,3)
%         plot(total_error(:,i,j))
%     end
% end