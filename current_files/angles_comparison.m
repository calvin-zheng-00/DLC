...
unprotemp = dir(fullfile(unpro_src,'*.csv'));
unprofolder = {unprotemp(~[unprotemp.isdir]).name};
...
for catfolder_i = 1:numel(catfolder)
    ...
    grasp_times = fullfile(time_src,timefolder{catfolder_i});
    grasp_times = readtable(grasp_times);
    ComparisonArray = [];
    unproTable = fullfile(unpro_src,unprofolder{catfolder_i});
    ...
    for trialfolder_j = 1:numel(trialfolder)
        ...
        grasp_time_3d = grasp_times.grasp_time(trialfolder_j);
        grasp_time = grasp_times.reach_time(trialfolder_j);
        % grasp_time = grasp_times.reach_time(trialfolder_j);
    ...
    outloc = fullfile("Comparison/",strcat(catfolder{catfolder_i},".csv"));
