...
src_3d = 'C:\Users\czhe0008\Documents\DLCprojects\openpose\data_conversion\massive_3d\filter_participant';
...
opt_src = 'C:\Users\czhe0008\Documents\MATLAB\Openpose\Optimisation\';
err_src = 'C:\Users\czhe0008\Documents\MATLAB\Openpose\Error\';
grasp_types = 'C:\Users\czhe0008\Documents\MATLAB\Openpose\grasp_type_2.csv';
...
for f = 1:numel(mainfolder)
    focus_cat = f;
    test = fullfile(src_3d,mainfolder{f});
    allavgerr = [];
    good_pc = [];
    pcs = 1:28;
    for d = 1:loops
        chosen = length(pcs)-opt_num;
        opt = nchoosek(pcs,chosen);
        loopnum = height(opt);
        avgerrcomp = [];
        for r = 1:loopnum
            ...
            for i = 1:numel(profolder)
                ...
                % end
                total_error = [];
                %% This for loop is to loop through the extra folder layer for participants
                acttemp = dir(fullfile(src_3d,mainfolder{i},'*'));
                actfolder = setdiff({acttemp([acttemp.isdir]).name},{'.','..'});
                for q = 1:numel(actfolder)
                    subtemp = dir(fullfile(src_3d,mainfolder{i},actfolder{q},'*.csv'));
                    ...
                    if focus_cat == 1
                        ...
                        % avg_error = mean(median(total_error(:,21),"omitnan"),"omitnan"); % Finger tips
                    elseif focus_cat == 2
        ...
        good_pc = [good_pc; [opt(idx,:),NaN(1,d)]];
    ...
    % ylabel('Error (mm)')
    % box off
    outputstr = strcat(opt_src,'\',mainfolder{f},'.txt');
    writematrix(good_pc,outputstr)
    outputstr = strcat(err_src,'\',mainfolder{f},'.txt');
    writematrix(allavgerr,outputstr)
end