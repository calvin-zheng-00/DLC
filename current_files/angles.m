...
src = 'C:\Users\czhe0008\Documents\DLCprojects\openpose\data_conversion\massive_3d\filter';
...
%             outloc = fullfile(outfolder,strcat(trialfolder{trialfolder_j}(1:end-13),"_angles.csv"));
        ...
        x = dot(v1,wrist_forward,2); % magnitude of the component of v1 along the index vector
        y = dot(v1,wrist_fe_axis,2); % magnitude of the component of v1 along the index vector
        df2.("W_abd") = pi/2-atan(y./x);
        df2.("W_flex") = pi/2-acos(z./sqrt(x.^2 + y.^2 + z.^2));
        ...
        %ref1 = normt;
        ref1 = wrist_fe_axis; 
        forearm_np = v1.*((ref1(:,1).*v1(:,1) + ref1(:,2).*v1(:,2) + ref1(:,3).*v1(:,3))./(v1mag.^2));   % Vector component normal to the palm plane
        forearm_p = [ref1(:,1)-forearm_np(:,1),ref1(:,2)-forearm_np(:,2),ref1(:,3)-forearm_np(:,3)];                                       % Vector component on the palm plane
